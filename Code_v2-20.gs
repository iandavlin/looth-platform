...
const CONFIG = {
  ...
  COL: {
    ...
    FEATURED_IMAGE:   21,
    OTHER_ATTENDEES:  22,
    ZOOM_URL:         23, // ← NEW (WP publish) — the gated virtual-attend link
  },
...
function onEdit(e) {
  ...
  if (wpUrl) {
    const wpRelevantCols = [
      ...
      CONFIG.COL.ZOOM_URL,   // ← NEW — Zoom edits on a live event prompt a re-publish
    ];
...
function publishRowToWp_(rowIndex, allowDraftOverride) {
  ...
  if (zoom)   payload.zoom_url = zoom;   // optional — omit = no virtual-attend CTA
...
function publishRowFromWebApp(rowIndex) {
  try {
    ...
    if (zoom)   wpPayload.zoom_url = zoom;   // optional — gated virtual-attend link
