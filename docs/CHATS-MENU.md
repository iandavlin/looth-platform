...
| 10 | **social** | *(pending spawn)* | `pending` | connections + messaging + notifications (in profile-app) | **TO SPAWN (Ian, 2026-05-30).** The BuddyPress social-layer rebuild, in-house. Builds ON the existing scaffold (`Connections.php`/`Messaging.php`/social API stubs/`migrate-social-from-bb.php`/`2026-05-30-social-layer.sql`). **CUT-DAY-REQUIRED.** Bootstrap `bootstrap-social-messaging.md`. Coordinates with profile-2.0 (shared `profile-app/`, on-`/u/` buttons), lg-shell (header modals call its backend), shim-replacement (flag before `Whoami.php`/`config.php`). Open: who-can-DM, ship-follow, counts endpoint. Capture session ID at spawn → coordinator. |
