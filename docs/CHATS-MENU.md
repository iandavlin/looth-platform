...
| 2 | **profile-app → profile-2.0** | *(fresh chat pending spawn)* | `a847d1aa` RETIRED → new pending | identity, `/whoami`, profile pages, directory, profile-2.0 block system | **`a847d1aa` RETIRED 2026-05-29** — shipped slice 0→3.5, auth bridge, cross-lane coord, + cutover-prep backfills (`23fe81b`: migrate-socials.php, location_address, linktree; handoff `76052eb`). **profile-2.0 awaits a FRESH chat** on `marching-orders-profile-2.0.md` (backfills done → start at block system; profile block system is SEPARATE from layoutv2, sidebar-palette composer). Parallel: shim-replacement design. |
...
| 9 | **shim-replacement** | *(pending spawn)* | *(pending)* | mint looth_id at WP login, retire per-page whoami loopback | NEW, **PRE-CUT REQUIRED** (Ian: fast first experience, no slow site day-one). Bootstrap `bootstrap-shim-replacement.md`. Owns: mint endpoint (profile-app) + WP login hooks + consumer-verify pattern + bb-mirror proof. Dev-built+soaked before flip (auth invariant). |
