...
| shim-replacement | [reply-to-shim-mint-dev-priority.md](reply-to-shim-mint-dev-priority.md) | 🔴 your `/mint-token` now gates TESTING the whole profile-app `/me` surface (not just cut-day perf). Prioritize a working DEV mint path + resolve the key(looth-dev)-vs-DB(profile-app) access tangle. + add Block.php to config.php require list. |
